...
class SessionBuilder {
  ...
  /// meaning plus distractors generated by [generateDistractors], which
  /// prefers wrong answers that share this card's grammatical `focus` and
  /// resembles it in surface form, and varies from attempt to attempt instead
  /// of offering the same three wrong answers every time a card recurs.
  List<String> choicesFor({
    ...
    // Same-unit cards first (thematically close, so a same-focus match among
    // them is doubly plausible); the generator itself falls back to the
    // whole course when a unit does not have enough same-focus material.
    final pool = unit?.cards ?? course.allCards;
    final wider = pool.length >= count ? pool : course.allCards;
    ...
    final options = <String>[card.native, ...wrong];
